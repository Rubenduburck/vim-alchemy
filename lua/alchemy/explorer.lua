...
function M.show_conversions_for_encoding(text_selection, input_encoding)
    local operations = {
        ...
        { type = "convert", name = "Convert to hex", action = function() return Core.convert(text_selection.text, input_encoding, "hex") end },
        { type = "convert", name = "Convert to int", action = function() return Core.convert(text_selection.text, input_encoding, "int") end },
        { type = "convert", name = "Convert to base64", action = function() return Core.convert(text_selection.text, input_encoding, "base64") end },
        { type = "convert", name = "Convert to base58", action = function() return Core.convert(text_selection.text, input_encoding, "base58") end },
        { type = "convert", name = "Convert to binary", action = function() return Core.convert(text_selection.text, input_encoding, "bin") end },
        { type = "convert", name = "Convert to UTF-8", action = function() return Core.convert(text_selection.text, input_encoding, "utf8") end },
        { type = "convert", name = "Convert to ASCII", action = function() return Core.convert(text_selection.text, input_encoding, "ascii") end },
        { type = "convert", name = "Convert to bytes", action = function() return Core.convert(text_selection.text, input_encoding, "bytes") end },
        
        -- Other operations (work on any format)
        { type = "operation", name = "Hash with SHA256", action = function() return M.hash_text(text_selection.text, "sha256") end },
        { type = "operation", name = "Hash with SHA512", action = function() return M.hash_text(text_selection.text, "sha512") end },
        { type = "operation", name = "Hash with MD5", action = function() return M.hash_text(text_selection.text, "md5") end },
        { type = "operation", name = "Hash with Blake2", action = function() return M.hash_text(text_selection.text, "blake2") end },
        { type = "operation", name = "Hash with Keccak256", action = function() return M.hash_text(text_selection.text, "keccak256") end },
    }
    
    -- Add array-specific operations if the text looks like an array
    if M.is_array_like(text_selection.text) then
        table.insert(operations, { type = "array", name = "Flatten array", action = function() return M.flatten_array(text_selection.text) end })
        table.insert(operations, { type = "array", name = "Chunk array", action = function() return M.chunk_array_interactive(text_selection.text) end })
        table.insert(operations, { type = "array", name = "Rotate array", action = function() return M.rotate_array_interactive(text_selection.text) end })
        table.insert(operations, { type = "array", name = "Pad left", action = function() return M.pad_left_interactive(text_selection.text) end })
        table.insert(operations, { type = "array", name = "Pad right", action = function() return M.pad_right_interactive(text_selection.text) end })
    end
